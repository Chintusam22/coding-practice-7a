...
const convetMatchDetailsDbObjectToResponseObeject = (dbObject) => {
  return {
...
app.get("/players/", async (request, response) => {
  const getPlayerQueryA = `
    SELECT
        *
    FROM 
        player_details;`;
  const playerArray = await db.all(getPlayerQueryA);
  response.send(
...
app.get("/matches/:matchId/", async (request, response) => {
  ...
  response.send(convetMatchDetailsDbObjectToResponseObeject(matchDetails));
});
...
app.get("/players/:playerId/matches/", async (request, response) => {
  ...
  const getPlayerMatchesQuery = `
    ...
    WHERE
      player_id = ${playerId};`;
  const playerMatchQuery = await db.all(getPlayerMatchesQuery);
  response.send(playerMatchQuery);
});
